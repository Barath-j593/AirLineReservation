...
def create_booking():
    try:
        ...
        max_points = min(available_points, total_price)
...
def admin_flights():
    ...
    return render_template(
        ...
        get_flight_status_badge_class=get_flight_status_badge_class
    )
...
def update_flight_status(flight_id):
    ...
    if new_status == 'Cancelled' and flight.status != 'Cancelled':
        ...
        for booking in bookings:
            ...
            if user:
                send_booking_cancellation_email(user, booking, flight, mail)
            flight.seats_available += len(booking.passengers)
